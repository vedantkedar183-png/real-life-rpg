...
const AI = {
  ...
  _buildContext() {
    ...
    const unlockedAch = Object.values(State.achievements).filter(a => a.unlocked).length;
...
Focus on: weakness to improve, strength to leverage, habit tip. No other text outside the JSON.`);
    ...
    if (!match) throw new Error('Could not parse insight cards');
  ...
  async getFocusQuest() {
    ...
    const questList = quests.map(q => `"${q.name}" (${q.category}, ${q.difficulty})`).join(', ');
    const raw = await this.generateContent(`${ctx}
...
{"quest": "exact quest name", "reason": "1-2 sentences using RPG language", "icon": "single emoji"}`);
  ...
  async chat(userMessage, history = []) {
    const ctx = this._buildContext();
    const historyText = history.slice(-6).map(m => `${m.role === 'user' ? 'Player' : 'Coach'}: ${m.text}`).join('\n');
    return this.generateContent(`${ctx}
